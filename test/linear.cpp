        ...
        */
        ...
        void doubled()
        {
            ...
            cout<<load<<endl;
                ...
                htable = ntable;
                */
            double load = current / tsize;
            
            if(load > FACTOR)
            {
                cout<<"table doubled"<<endl;
                int oldTableSize = tsize;
                tsize = findNextPrime(oldTableSize*2);
                ...
                for (int hash = 0; hash < oldTableSize; hash++)
                    ...
                    {
                        HashNode *oldEntry;
                        HashNode *entry = oldTable[hash];
                        
                        rehash(entry->key, entry->value, entry->gpa);
                        oldEntry = entry;
                        delete oldEntry;
        ...
        string print()
        {
            ...
            /*for(int i = 0; i<tsize-1; i++)
            {
                ...
                int perm = htable[i]->key;
                string name = htable[i]->value;
                double score = htable[i]->gpa;
